...
//		}
    ...
    public static String generateToFile(GenTemplate tpl, Map<String, Object> model, boolean isReplaceFile) {
        ...
        String html = fileName.substring(fileName.length() - 4, fileName.length());
        if ("html".equals(html)) {
            content = content.replace("<%@ page contentType=\"text/html;charset=UTF-8\" %>", "");
            content = content.replace("<%@ include file=\"/WEB-INF/views/include/taglib.jsp\"%>", "<#include \"/include/taglib.html\">\n");
